...
class BellmanNode
{
    ...
    BellmanNode(int _u, int _v, int _w) { u = _u; v = _v; weight = _w; }
...
class BellmanMain
{
    void bellmanFord(ArrayList<BellmanNode> edges, int N, int src)
    {
        ...
        for(int i = 1;i<=N-1;i++) {
            for(BellmanNode BellmanNode : edges) {
                if(dist[BellmanNode.getU()] + BellmanNode.getWeight() < dist[BellmanNode.getV()]) {
                    dist[BellmanNode.getV()] = dist[BellmanNode.getU()] + BellmanNode.getWeight();
                }
            }
        }
    ...
    public static void main(String args[])
    {
        ...
        BellmanMain obj = new BellmanMain();
